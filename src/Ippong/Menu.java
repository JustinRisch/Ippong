...
public class Menu extends JMenuBar {
	...
	public Menu(Ball ball, ArrayList<Brick> bricks, Paddle paddle,
			JPanel contentPane) {
		...
		item.addActionListener(e -> {
			...
			icon.getIconWidth();
			paddle.setBounds(paddle.getX(), paddle.getY(), icon.getIconWidth(),
					paddle.getHeight());
			paddle.setIcon(icon);
			ImageIcon ballIcon = new ImageIcon("images/photo.png");

			ball.setIcon(ballIcon);
			ball.setBounds(ball.getX(), ball.getY(), ballIcon.getIconHeight(),
					ballIcon.getIconHeight());
			ball.revalidate();
			ball.repaint();
			bricks.forEach(x -> {
				x.setText("[C#]");
